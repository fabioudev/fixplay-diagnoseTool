<script lang="ts">
  ...
  let {
    ...
    open: boolean;
    manager: {
      calibrateSticksBegin: () => Promise<void>;
      calibrateSticksSample: () => Promise<void>;
      calibrateSticksEnd: () => Promise<void>;
      calibrateRangeBegin: () => Promise<void>;
      calibrateRangeEnd: () => Promise<void>;
      getInMemoryModuleData: () => Promise<number[] | null>;
    } | null;
  ...
  function sampleStick(x: number, y: number, bins: number[]) {
    ...
    const bin =
      Math.floor(((angle + Math.PI) / (2 * Math.PI)) * CIRCULARITY_DATA_SIZE) %
      CIRCULARITY_DATA_SIZE;
  ...
  async function runCenterCalibration() {
    ...
    try {
      ...
      if (manager) {
        afterData = await manager.getInMemoryModuleData().catch(() => null);
      }
  ...
  async function finishRangeCalibration() {
    ...
    try {
      ...
      statusText = get(LL).calibration.rangeDone({
        left: leftErr.toFixed(1),
        right: rightErr.toFixed(1),
      });
  ...
  async function start() {
    ...
    if (manager) {
      beforeData = await manager.getInMemoryModuleData().catch(() => null);
    }
    if (mode === 'center') {
      totalSteps = 6;
    ...
    >
      <div class="mb-4 flex items-center justify-between">
        ...
        <button
          ...
          onclick={close}
          aria-label={$LL.calibration.closeAria()}
        >
          <X class="h-5 w-5" />
        </button>
      </div>
      ...
      <div class="mb-4 flex gap-2">
        <button
          class="flex-1 rounded-lg px-3 py-2 text-sm font-medium transition {mode === 'center'
        ...
        <button
          class="flex-1 rounded-lg px-3 py-2 text-sm font-medium transition {mode === 'range'
            ? 'bg-blue-600 text-white'
            : 'bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-200'}"
          onclick={() => (mode = 'range')}
          disabled={busy}
      ...
      {#if success}
        <div
          class="mb-4 rounded-lg bg-green-50 p-3 text-sm text-green-700 dark:bg-green-900/30 dark:text-green-300"
        >
          ✓ {statusText}
        </div>
      {/if}
        ...
        <div class="mb-4 rounded-lg border border-slate-200 p-3 dark:border-slate-600">
          <div class="mb-2 flex items-center justify-between">
            <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-200">
              {$LL.calibration.beforeAfter()}
            </h3>
            <span class="text-xs text-slate-500 dark:text-slate-400"
              >{$LL.calibration.valuesChanged({ changed })}</span
            >
          </div>
          <div class="grid grid-cols-12 gap-1 text-center text-[10px] font-mono">
            {#each calibDiff as d, i (i)}
              <div
                ...
                title={$LL.calibration.valueTitle({
                  index: i + 1,
                  before: d.before,
                  after: d.after,
      ...
      <div class="flex justify-end gap-2">
        {#if mode === 'range' && step === 2 && !busy}
          <button
            ...
            onclick={finishRangeCalibration}
          >
            {$LL.calibration.done()}
          </button>
        {/if}
