...
{#if $updateAvailable && !$updateDismissed}
  <div
    class="flex items-center gap-3 px-4 py-2 bg-emerald-950/70 border-b border-emerald-700/50 text-emerald-100 text-xs"
  >
    ...
    <div class="flex-1 min-w-0">
      ...
      <span class="text-emerald-300/70"
        >{$LL.update.currentSuffix({ current: $updateAvailable.currentVersion })}</span
      >
      {#if $updateError}
    ...
    {#if managed}
      <span class="text-emerald-200/90 hidden sm:inline"
        >{$LL.update.managedHint({ hint: hint || $LL.update.packageManager() })}</span
      >
      <a
    ...
    <button
      onclick={() => {
        ...
        try {
          localStorage.setItem('fixplay-update-remind', String(expiry));
        } catch {
          /* ignored */
        }
      ...
      title={$LL.update.laterTitle()}>{$LL.update.later()}</button
    >
    <button
...
{:else if $updateError && onCheck}
  ...
  <div
    class="flex items-center gap-3 px-4 py-1.5 bg-red-950/60 border-b border-red-800/50 text-red-200 text-xs"
  >
    <span class="flex-1">{$LL.update.checkFailed({ error: $updateError })}</span>
    <button
      onclick={onCheck}
      class="px-2 py-0.5 rounded bg-red-700/50 hover:bg-red-700 transition-colors"
      >{$LL.update.retry()}</button
    >
  </div>
{/if}
