...
>
  ...
  <g
    transform="translate(44, -24) scale(0.30)"
    fill="url(#shell)"
    stroke="#1f2937"
    stroke-width="5"
  >
  ...
  <rect x="150" y="2" width="120" height="4" rx="2" fill={lightCss} style="filter: {lightGlow}" />
  ...
  <rect
    ...
    height={TP.h}
    rx="4"
    fill={btnFill('touchpad')}
    stroke={btnStroke('touchpad')}
    stroke-width="1.2"
  />
  {#each [0, 1] as i (i)}
    {#if touchDot(i)}
  ...
  <rect
    ...
    height="5"
    rx="2.5"
    fill={btnFill('options')}
    stroke={btnStroke('options')}
    stroke-width="1"
  />
  ...
  <g transform="translate(324 83)">
    <circle
      ...
      cy="-18"
      r="10"
      fill={faceFill('triangle')}
      stroke={faceStroke('triangle')}
      stroke-width="1.4"
      style={pressed('triangle') ? 'filter:url(#viz-glow)' : ''}
    />
    <path
      d="M0,-24 L5,-15 L-5,-15 Z"
      fill="none"
      stroke={pressed('triangle') ? '#0b1220' : '#4b5563'}
      stroke-width="1.2"
    ...
    <circle
      ...
      cy="0"
      r="10"
      fill={faceFill('circle')}
      stroke={faceStroke('circle')}
      stroke-width="1.4"
    ...
    <circle
      ...
      r="4.5"
      fill="none"
      stroke={pressed('circle') ? '#0b1220' : '#4b5563'}
      stroke-width="1.2"
    />
    <circle
      cx="0"
      ...
      r="10"
      fill={faceFill('cross')}
      stroke={faceStroke('cross')}
      stroke-width="1.4"
      style={pressed('cross') ? 'filter:url(#viz-glow)' : ''}
    />
    <path
      d="M-4,14 L4,22 M4,14 L-4,22"
      fill="none"
      stroke={pressed('cross') ? '#0b1220' : '#4b5563'}
      stroke-width="1.2"
    ...
    <circle
      ...
      cy="0"
      r="10"
      fill={faceFill('square')}
      stroke={faceStroke('square')}
      stroke-width="1.4"
    ...
    <rect
      ...
      rx="1.5"
      fill="none"
      stroke={pressed('square') ? '#0b1220' : '#4b5563'}
      stroke-width="1.2"
    />
  </g>
  ...
  <g>
    {#each [{ side: 'left' as const, cx: LEFT_CX, cy: LEFT_CY }, { side: 'right' as const, cx: RIGHT_CX, cy: RIGHT_CY }] as s (s.side)}
      ...
      <circle
        ...
        r={STICK_R}
        fill="#0b1220"
        stroke={pressed(l3r3) ? '#5eecd9' : '#374151'}
      ...
      >
      <circle
        cx={stickDot(s.side).cx}
        cy={stickDot(s.side).cy}
        r={DOT_R}
        fill="#5eecd9"
        opacity="0.9"
      />
  ...
  <text
    ...
    text-anchor="middle"
    class="ps-glyph"
    fill={pressed('ps') ? '#0b1220' : '#9ca3af'}>PS</text
  >
  <circle
    cx="210"
    cy="153"
    r="3"
    fill={pressed('mute') ? '#fbbf24' : '#1f2937'}
    stroke={pressed('mute') ? '#fde68a' : '#374151'}
    stroke-width="0.8"
  ...
  <g transform="translate(210 170)">
    ...
    <rect
      x="-9"
      y="-3"
      width={18 * ($batteryStatus.charge_level / 100)}
      height="6"
      rx="1"
      fill={$batteryStatus.charge_level > 20
        ? '#22c55e'
        : $batteryStatus.charge_level > 10
          ? '#f59e0b'
          : '#ef4444'}
      opacity="0.8"
    />
    <text x="0" y="14" text-anchor="middle" class="lbl-sm" fill="#6b7280"
      >{$batteryStatus.charge_level}%</text
    >
  </g>
...
<style>
  ...
  .controller-viz .lbl-sm {
    font:
      500 7px ui-sans-serif,
      system-ui,
      sans-serif;
  }
  .controller-viz .ps-glyph {
    font:
      700 6px ui-sans-serif,
      system-ui,
      sans-serif;
  }
</style>
