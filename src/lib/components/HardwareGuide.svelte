    ...
    <div class="px-3 pb-3 pt-1 space-y-4 text-xs text-gray-300" transition:slide={{ duration: 150 }}>
      ...
      {#if variant === 'ch341a'}
        ...
        <div>
          ...
          <div class="flex flex-wrap items-start gap-4">
            <svg viewBox="0 0 340 210" class="w-full max-w-md shrink-0" role="img" aria-label="CH341A board top view">
              <!-- PCB outline -->
              <rect x="18" y="26" width="304" height="158" rx="7" fill="#0b0f1a" stroke="#374151" stroke-width="1.5" />
              <!-- USB-A male connector (sticks out left) -->
              <rect x="6" y="86" width="16" height="26" fill="#cbd5e1" stroke="#94a3b8" stroke-width="1" />
              <rect x="9" y="91" width="10" height="16" fill="#1f2937" />
              <!-- 24-pin ZIF socket (green) -->
              <rect x="96" y="44" width="150" height="120" rx="3" fill="#14532d" stroke="#16a34a" stroke-width="1.5" />
              <!-- ZIF lever (the pull handle) -->
              <rect x="110" y="36" width="120" height="7" rx="2" fill="#d1d5db" stroke="#9ca3af" stroke-width="1" />
              <rect x="158" y="32" width="24" height="13" rx="3" fill="#9ca3af" stroke="#6b7280" stroke-width="1" />
              <!-- ZIF pin holes: 2 columns x 12 rows -->
              {#each Array(12) as _, row (row)}
                {#each [110, 232] as cx (cx)}
                  <circle cx={cx} cy={58 + row * 8.2} r="1.8" fill="#022c22" stroke="#374151" stroke-width="0.6" />
                {/each}
              {/each}
              ...
              {/each}
              <!-- CH341A IC (SOIC-28) -->
              <rect x="34" y="116" width="22" height="48" rx="1" fill="#1f2937" stroke="#6b7280" stroke-width="1" />
              <circle cx="38" cy="122" r="1.4" fill="#cbd5e1" />
              ...
              <!-- Power LED -->
              <circle cx="40" cy="164" r="2.6" fill="#4ade80" stroke="#16a34a" stroke-width="0.6" />
              <!-- ACT# jumper (2-pin header + yellow shunt) -->
              ...
              <!-- SPI/I2C mode jumper (yellow shunt) -->
              ...
              <!-- Side headers (7-pin each side of the ZIF) -->
              ...
              {#each BOARD_CALLOUTS as c (c.n)}
                <circle cx={c.x} cy={c.y} r="7.5" fill="#111827" stroke="#f59e0b" stroke-width="1.2" />
        ...
        <div>
          ...
          <div class="flex flex-wrap items-start gap-4">
            <svg viewBox="0 0 160 130" class="w-44 shrink-0" role="img" aria-label="SOIC-8 SPI NOR pinout">
              <rect x="40" y="14" width="80" height="100" rx="3" fill="#1f2937" stroke="#4b5563" stroke-width="1.5" />
              <circle cx="50" cy="24" r="2.5" fill="#6b7280" />
              <text x="80" y="68" text-anchor="middle" class="chip-lbl" fill="#9ca3af">25-series</text>
              {#each L8 as p (p.n)}
                <rect x="28" y={20 + (p.n - 1) * 22} width="14" height="6" rx="1" fill="#6b7280" />
              ...
              {/each}
              {#each R8 as p, i (p.n)}
                <rect x="118" y={20 + i * 22} width="14" height="6" rx="1" fill="#6b7280" />
        ...
        <div>
          ...
          <div class="flex flex-wrap items-start gap-4">
            <svg viewBox="0 0 160 210" class="w-44 shrink-0" role="img" aria-label="SOIC-16 SPI NOR pinout">
              <rect x="40" y="14" width="80" height="180" rx="3" fill="#1f2937" stroke="#4b5563" stroke-width="1.5" />
              <circle cx="50" cy="24" r="2.5" fill="#6b7280" />
              <text x="80" y="108" text-anchor="middle" class="chip-lbl" fill="#9ca3af">25-series 16P</text>
              {#each L16 as p (p.n)}
                <rect x="28" y={20 + (p.n - 1) * 20} width="14" height="5" rx="1" fill="#6b7280" />
              ...
              {/each}
              {#each R16 as p, i (p.n)}
                <rect x="118" y={20 + i * 20} width="14" height="5" rx="1" fill="#6b7280" />
          ...
          <table class="text-[11px] border-collapse w-full max-w-md">
            <thead>
              <tr class="text-gray-500">
                <th class="text-left font-medium py-0.5 pr-2">Chip-Pin</th>
                <th class="text-left font-medium py-0.5 pr-2">Signal</th>
                <th class="text-left font-medium py-0.5">ZIF / CH341A</th>
              </tr>
            </thead>
            <tbody>
              {#each ZIF_WIRING as row (row.n)}
                <tr>
                  <td class="py-0.5 pr-2 font-mono text-teal-300">{row.n} {row.name}</td>
                  <td class="py-0.5 pr-2 font-mono text-gray-200">{row.sig}</td>
                  <td class="py-0.5 text-gray-400">
                    {#if row.note}<span class="text-amber-300">{row.note}</span>{/if}
                  </td>
                </tr>
              {/each}
            </tbody>
