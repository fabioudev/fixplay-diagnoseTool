...
{#if open}
  ...
  >
    <div
      class="w-full max-w-sm rounded-2xl bg-gray-800 p-6 shadow-2xl border border-gray-700 text-center"
      use:trapFocus
      transition:scale={{ duration: 150, start: 0.96 }}
    >
      <button
        class="absolute top-3 right-3 text-gray-500 hover:text-gray-300"
        onclick={() => (open = false)}
      >
        <X class="h-5 w-5" />
      </button>
      ...
      <div class="mt-4 flex justify-center gap-3">
        <a
          href="https://github.com/fabioudev/fixplay-diagnoseTool"
          target="_blank"
          rel="noopener"
        ...
        <a
          href="https://github.com/fabioudev/fixplay-diagnoseTool/wiki"
          target="_blank"
          rel="noopener"
          class="flex items-center gap-1 text-xs text-teal-400 hover:text-teal-300"
        >
      ...
      <button
        class="mt-4 rounded-lg bg-gray-700 px-6 py-2 text-sm text-gray-300 hover:bg-gray-600"
        onclick={() => (open = false)}
      >
        {$LL.about.close()}
      </button>
