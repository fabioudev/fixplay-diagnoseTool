<script lang="ts">
  ...
  async function browseArchiveDir() {
    const result = await openDialog({
      title: get(LL).settings.archiveBrowseTitle(),
      directory: true,
    });
    if (result && typeof result === 'string') {
  ...
  async function checkForUpdates() {
    ...
    try {
      ...
      updateCheckMsg = get(LL).settings.updateCheckFailedMsg({
  ...
  >
    <div class="flex items-center justify-between px-4 py-3 border-b border-gray-700">
      ...
      <button
        ...
        class="text-gray-400 hover:text-gray-200 text-lg leading-none">✕</button
      >
    </div>
    ...
    <div class="flex flex-col gap-6 p-4 overflow-y-auto flex-1">
      ...
      <div class="flex flex-col gap-1.5">
        ...
        {#if flashromStatus}
          <div
            class="flex items-center gap-1.5 text-xs {flashromStatus.ok
              ? 'text-emerald-400'
              : 'text-amber-400'}"
            title={flashromStatus.path}
          >
            ...
            {:else}
              ...
              <span class="truncate" title={flashromStatus.reason ?? ''}
                >{flashromStatus.reason ?? $LL.settings.flashromNotFound()}</span
              >
            {/if}
            <button
              onclick={refreshFlashrom}
              class="ml-auto text-gray-500 hover:text-gray-300 shrink-0"
              title={$LL.settings.recheck()}
            >
        ...
        <div class="relative">
          ...
          >
            {#each BAUD_RATES as rate (rate)}
              <option value={rate}
                >{rate}{rate === 115200 ? $LL.settings.baudStandardPs5() : ''}</option
              >
            {/each}
          </select>
          <span
        ...
        <div class="relative">
          <select
            ...
            onchange={async (e) => {
              appSettings.update((s) => ({
                ...s,
                i2c_baud_rate: Number((e.target as HTMLSelectElement).value),
              }));
              await save();
            }}
          ...
          >
            {#each BAUD_RATES as rate (rate)}
              <option value={rate}
                >{rate}{rate === 115200 ? $LL.settings.baudStandardPico() : ''}</option
              >
            {/each}
          </select>
          <span
        ...
        >
          <input
            ...
            onchange={async (e) => {
              appSettings.update((s) => ({
                ...s,
                tablet_mode: (e.target as HTMLInputElement).checked,
              }));
              await save();
            }}
