<script lang="ts">
  ...
  async function applyLights(): Promise<void> {
    ...
    try {
      ...
      pushControllerLog(
        get(LL).tester.lightsLog({ r: rr, g: gg, b: bb, leds: ledPattern(), mute: muteMode }),
        'info'
      );
    } catch (e) {
      pushControllerLog(
        get(LL).tester.lightsFailed({ error: e instanceof Error ? e.message : String(e) }),
        'error'
      );
    }
  }
  ...
  async function resetLights(): Promise<void> {
    ...
    try {
      ...
      pushControllerLog(
        get(LL).tester.lightsResetFailed({ error: e instanceof Error ? e.message : String(e) }),
        'error'
      );
    }
  }
  ...
  async function applyVibration(): Promise<void> {
    ...
    try {
      ...
      pushControllerLog(
        get(LL).tester.vibFailed({ error: e instanceof Error ? e.message : String(e) }),
        'error'
      );
    }
  }
  ...
  async function applyTriggers(): Promise<void> {
    ...
    try {
      ...
      pushControllerLog(
        get(LL).tester.triggerFailed({ error: e instanceof Error ? e.message : String(e) }),
        'error'
      );
    }
  }
  ...
  async function runLightsPattern(): Promise<void> {
    ...
    try {
      ...
      pushControllerLog(
        get(LL).tester.lightsPatternFailed({ error: e instanceof Error ? e.message : String(e) }),
        'error'
      );
    } finally {
      lightsRunning = false;
  ...
  async function runVibPattern(): Promise<void> {
    ...
    try {
      ...
      pushControllerLog(
        get(LL).tester.vibPatternFailed({ error: e instanceof Error ? e.message : String(e) }),
        'error'
      );
    } finally {
      vibRunning = false;
  ...
  async function runTriggerPattern(): Promise<void> {
    ...
    try {
      ...
      for (const m of modes) {
        ...
        const cfg: AdaptiveTriggerConfig = {
          mode: m.mode,
          start: m.start,
          end: m.end,
          force: m.force,
      ...
      pushControllerLog(
        get(LL).tester.triggerPatternFailed({ error: e instanceof Error ? e.message : String(e) }),
        'error'
      );
    } finally {
      triggerRunning = false;
  ...
  async function playSpeakerTone(durationMs: number): Promise<void> {
    ...
    try {
      ...
      pushControllerLog(
        get(LL).tester.speakerFailed({ error: e instanceof Error ? e.message : String(e) }),
        'error'
      );
    } finally {
  ...
  {#if tab === 'lights'}
    ...
    <div class="flex flex-col gap-3">
      <div class="flex items-center gap-4">
        <div class="flex flex-col gap-2 flex-1">
          ...
          <div class="flex items-center gap-2">
            ...
            <input
              ...
              max="100"
              value={brightness}
              oninput={(e) => (brightness = parseFloat((e.target as HTMLInputElement).value))}
        ...
        <div
          class="w-16 h-16 rounded-lg border border-gray-600 shrink-0"
          style="background: rgb({scaled(r)},{scaled(g)},{scaled(b)})"
        ></div>
      </div>
      ...
      <div class="flex flex-wrap gap-1.5">
        {#each LIGHT_PRESETS as p (p.id)}
          <button
            class="rounded px-2 py-1 text-xs bg-gray-700/60 hover:bg-gray-600 text-gray-300"
            onclick={() => {
              r = p.r;
              g = p.g;
              b = p.b;
        ...
        <div class="flex items-center gap-2 flex-wrap">
          {#each leds as on, i (i)}
            <button
              ...
              class="w-7 h-7 rounded-full border text-xs font-medium {on
                ? 'bg-teal-500/30 border-teal-400 text-teal-200'
                : 'bg-gray-700/60 border-gray-600 text-gray-500'}">{i + 1}</button
            >
          {/each}
          <span class="text-xs text-gray-600 ml-1"
            >{$LL.tester.pattern()}: 0b{ledPattern().toString(2).padStart(5, '0')}</span
          >
        </div>
        <div class="flex flex-wrap gap-1.5 mt-2">
          {#each PLAYER_PRESETS as p (p.id)}
            <button
              class="rounded px-2 py-0.5 text-xs bg-gray-700/60 hover:bg-gray-600 text-gray-300"
              onclick={() => {
                leds = [0, 1, 2, 3, 4].map((i) => ((p.pattern >> i) & 1) === 1);
              }}>{p.label($LL)}</button
            >
          {/each}
        </div>
      ...
      <div class="border-t border-gray-700 pt-3 flex items-center gap-2">
        ...
        {#each [{ id: 0, label: (ll: TranslationFunctions) => ll.tester.muteOff() }, { id: 1, label: (ll: TranslationFunctions) => ll.tester.muteOn() }, { id: 2, label: (ll: TranslationFunctions) => ll.tester.mutePulse() }] as m (m.id)}
          <button
            class="rounded px-2 py-0.5 text-xs {muteMode === m.id
              ? 'bg-amber-500/30 text-amber-200 border border-amber-400/50'
              : 'bg-gray-700/60 text-gray-400 hover:bg-gray-600'}"
            onclick={() => (muteMode = m.id as 0 | 1 | 2)}>{m.label($LL)}</button
          >
        {/each}
      </div>
      ...
      <div class="flex gap-2 pt-2">
        <button
          ...
          onclick={applyLights}><Play class="h-4 w-4" /> {$LL.tester.apply()}</button
        >
        {#if lightsRunning}
          <button
            class="flex items-center gap-1.5 rounded-lg bg-red-600/20 px-4 py-2 text-sm text-red-400 hover:bg-red-600/30"
            onclick={stopPattern}><Square class="h-4 w-4" /> {$LL.tester.stop()}</button
          >
        {:else}
          <button
            class="flex items-center gap-1.5 rounded-lg bg-amber-600/20 px-4 py-2 text-sm text-amber-400 hover:bg-amber-600/30"
            onclick={runLightsPattern}><Play class="h-4 w-4" /> {$LL.tester.testPattern()}</button
          >
        {/if}
        <button
          class="flex items-center gap-1.5 rounded-lg bg-gray-700 px-4 py-2 text-sm text-gray-300 hover:bg-gray-600"
          onclick={resetLights}><RotateCcw class="h-4 w-4" /> {$LL.tester.reset()}</button
        >
      </div>
    </div>
  {:else if tab === 'vibration'}
    <div class="flex flex-col gap-3">
      ...
      {#each VIB_SLIDERS as vib (vib.label)}
        <div class="flex items-center gap-2">
          ...
          <input
            ...
            max="255"
            value={vib.get()}
            oninput={(e) => vib.set(parseFloat((e.target as HTMLInputElement).value))}
            class="flex-1 accent-teal-500"
          />
      ...
      <div class="flex flex-wrap gap-2 pt-1">
        <button
          class="flex items-center gap-1.5 rounded-lg bg-teal-600 px-4 py-2 text-sm text-white hover:bg-teal-700"
          onclick={applyVibration}><Play class="h-4 w-4" /> {$LL.tester.start()}</button
        >
        <button
          class="flex items-center gap-1.5 rounded-lg bg-amber-600/20 px-4 py-2 text-sm text-amber-400 hover:bg-amber-600/30"
          onclick={runPulse}><Vibrate class="h-4 w-4" /> {$LL.tester.pulse()}</button
        >
        <button
          class="flex items-center gap-1.5 rounded-lg bg-red-600/20 px-4 py-2 text-sm text-red-400 hover:bg-red-600/30"
          onclick={stopVibration}><Square class="h-4 w-4" /> {$LL.tester.stop()}</button
        >
        {#if vibRunning}
          <button
            class="flex items-center gap-1.5 rounded-lg bg-red-600/20 px-4 py-2 text-sm text-red-400 hover:bg-red-600/30"
            onclick={stopPattern}><Square class="h-4 w-4" /> {$LL.tester.stopPattern()}</button
          >
        {:else}
          <button
            class="flex items-center gap-1.5 rounded-lg bg-purple-600/20 px-4 py-2 text-sm text-purple-400 hover:bg-purple-600/30"
            onclick={runVibPattern}><Play class="h-4 w-4" /> {$LL.tester.testPattern()}</button
          >
        {/if}
      </div>
    </div>
  {:else if tab === 'trigger'}
    <div class="flex flex-col gap-3">
        ...
        <div class="rounded-lg bg-gray-900/40 p-3 border border-gray-700">
          <div class="flex items-center justify-between mb-2">
            ...
            <span class="text-xs text-gray-500"
              >{$LL.tester.input()}:
              <span class="text-teal-300 tabular-nums">{getLevel()}</span>/255</span
            >
          </div>
          <div class="flex items-center gap-1.5 mb-2">
            {#each TRIGGER_MODES as m (m.id)}
              <button
                class="rounded px-2 py-0.5 text-xs {getMode() === m.id
                  ? 'bg-teal-500/30 text-teal-200 border border-teal-400/50'
                  : 'bg-gray-700/60 text-gray-400 hover:bg-gray-600 border border-transparent'}"
                onclick={() => setMode(m.id)}>{m.label($LL)}</button
              >
            {/each}
          </div>
          {#if getMode() !== 'off'}
            {#if getMode() === 'resistance'}
              <div class="flex items-center gap-2 mb-1">
                ...
                <input
                  ...
                  max="255"
                  value={getStart()}
                  oninput={(e) => setStart(parseFloat((e.target as HTMLInputElement).value))}
                  class="flex-1 accent-teal-500"
                />
              ...
              <div class="flex items-center gap-2">
                ...
                <input
                  ...
                  max="255"
                  value={getForce()}
                  oninput={(e) => setForce(parseFloat((e.target as HTMLInputElement).value))}
                  class="flex-1 accent-teal-500"
                />
            ...
            {:else if getMode() === 'single'}
              ...
              <div class="flex items-center gap-2 mb-1">
                ...
                <input
                  ...
                  max="255"
                  value={getEnd()}
                  oninput={(e) => setEnd(parseFloat((e.target as HTMLInputElement).value))}
                  class="flex-1 accent-teal-500"
                />
      ...
      <div class="flex gap-2 pt-1">
        <button
          class="flex items-center gap-1.5 rounded-lg bg-teal-600 px-4 py-2 text-sm text-white hover:bg-teal-700"
          onclick={applyTriggers}><Play class="h-4 w-4" /> {$LL.tester.apply()}</button
        >
        {#if triggerRunning}
          <button
            class="flex items-center gap-1.5 rounded-lg bg-red-600/20 px-4 py-2 text-sm text-red-400 hover:bg-red-600/30"
        ...
        {:else}
          <button
            class="flex items-center gap-1.5 rounded-lg bg-amber-600/20 px-4 py-2 text-sm text-amber-400 hover:bg-amber-600/30"
            onclick={runTriggerPattern}><Play class="h-4 w-4" /> {$LL.tester.testPattern()}</button
          >
        {/if}
        <button
          class="flex items-center gap-1.5 rounded-lg bg-gray-700 px-4 py-2 text-sm text-gray-300 hover:bg-gray-600"
          onclick={resetTriggers}><RotateCcw class="h-4 w-4" /> {$LL.tester.reset()}</button
        >
      </div>
    </div>
  {:else if tab === 'speaker'}
    <div class="flex flex-col gap-3">
      ...
      <div class="border-t border-gray-700 pt-2">
        ...
        <div class="flex gap-2 mt-2">
          {#if micActive}
            <button
              class="flex items-center gap-1.5 rounded-lg bg-red-600/20 px-3 py-1.5 text-xs text-red-400 hover:bg-red-600/30"
              onclick={() => (micActive = false)}
              ><Square class="h-3.5 w-3.5" /> {$LL.tester.levelOff()}</button
            >
          {:else}
            <button
              class="flex items-center gap-1.5 rounded-lg bg-teal-600/20 px-3 py-1.5 text-xs text-teal-400 hover:bg-teal-600/30"
              onclick={() => (micActive = true)}
              ><Mic class="h-3.5 w-3.5" /> {$LL.tester.levelOn()}</button
            >
          {/if}
        </div>
      </div>
      ...
      <div class="border-t border-gray-700 pt-2">
        ...
        <div class="flex gap-2 mt-2">
          {#if spectrumActive}
            <button
              class="flex items-center gap-1.5 rounded-lg bg-red-600/20 px-3 py-1.5 text-xs text-red-400 hover:bg-red-600/30"
              onclick={() => (spectrumActive = false)}
              ><Square class="h-3.5 w-3.5" /> {$LL.tester.spectrumOff()}</button
            >
          {:else}
            <button
              class="flex items-center gap-1.5 rounded-lg bg-teal-600/20 px-3 py-1.5 text-xs text-teal-400 hover:bg-teal-600/30"
              onclick={() => (spectrumActive = true)}
              ><Mic class="h-3.5 w-3.5" /> {$LL.tester.spectrumOn()}</button
            >
          {/if}
        </div>
      </div>
      ...
      <div class="border-t border-gray-700 pt-2">
        ...
        <div class="flex flex-wrap gap-2">
          ...
          {#if speakerPlaying}
            <button
              class="flex items-center gap-1.5 rounded-lg bg-red-600/20 px-3 py-1.5 text-xs text-red-400 hover:bg-red-600/30"
              onclick={stopSpeaker}><Square class="h-3.5 w-3.5" /> {$LL.tester.stop()}</button
            >
          {/if}
        </div>
      </div>
