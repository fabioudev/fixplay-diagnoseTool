<script lang="ts">
  ...
  async function run<T>(label: string, fn: () => Promise<T>): Promise<T | undefined> {
    if (!$i2cConnected) { pushLog(get(LL).i2c.notConnected(), 'error'); return undefined; }
    busy = label;
    try {
      const res = await fn();
      return res;
  ...
  async function fetchInfo() {
    ...
    if (i) {
      info = i;
      pushLog(get(LL).i2c.infoResult({ fw: i.firmware, bus: i.bus, scl: i.scl, sda: i.sda, voltage: i.voltage ? ' ' + i.voltage : '' }), 'status');
    }
  }
  ...
  async function doRead() {
    ...
    if (addr === null || len <= 0 || len > 4096) { pushLog(get(LL).i2c.invalidReadParams(), 'error'); return; }
    const reg  = readReg.trim() === '' ? null : parseHexByte(readReg);
    if (readReg.trim() !== '' && reg === null) { pushLog(get(LL).i2c.invalidRegister(), 'error'); return; }
  ...
  async function doReadEeprom() {
    ...
    if (addr === null || offset < 0 || offset > 0xffff || len <= 0 || len > 65535) {
      pushLog(get(LL).i2c.invalidEepromParams(), 'error'); return;
    }
  ...
  async function doWrite() {
    ...
    if (addr === null) { pushLog(get(LL).i2c.invalidWriteAddr(), 'error'); return; }
    if (writeReg.trim() !== '' && reg === null) { pushLog(get(LL).i2c.invalidRegister(), 'error'); return; }
    if (data === null || data.length === 0) { pushLog(get(LL).i2c.invalidData(), 'error'); return; }
    const ok = await run('Write', () => i2cWrite(addr, reg, data));
    if (ok !== undefined) {
      pushLog(get(LL).i2c.writeResult({ addr: hex(addr), reg: reg !== null ? ' @0x' + hex(reg) : '', count: data.length }), 'status');
    }
  }
...
<section class="flex flex-col gap-4 flex-1 bg-gray-900 rounded-lg p-4 min-h-0">
  ...
  <div class="flex flex-wrap items-center gap-2">
    ...
    <div class="flex items-center gap-2 ml-auto">
      {#if $xboxDbLoading}
        <span class="text-xs text-gray-500 flex items-center gap-1" title={$LL.i2c.dbLoadingTitle()}>
          <span class="inline-block animate-spin">⟳</span> {$LL.i2c.loadingDb()}
        </span>
  ...
  <div class="flex items-center gap-2">
    ...
    <span class="text-xs text-gray-400">
      {#if $i2cConnected}{$LL.i2c.statusConnected({ port: selectedPort })}{:else}{$LL.i2c.statusDisconnected()}{/if}
    </span>
  </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
    ...
    <div class="flex flex-col gap-1.5 bg-gray-950 rounded p-2.5">
      ...
      <div class="flex gap-1.5 text-xs">
        <input bind:value={readAddr} placeholder="Addr 0x48" title={$LL.i2c.addrTitle()} class="flex-1 w-20 bg-gray-800 text-gray-100 rounded px-2 py-1 border border-gray-700 focus:outline-none font-mono">
        <input bind:value={readReg} placeholder="Reg 0x00" title={$LL.i2c.regTitle()} class="flex-1 w-20 bg-gray-800 text-gray-100 rounded px-2 py-1 border border-gray-700 focus:outline-none font-mono">
        <input type="number" bind:value={readLen} min="1" max="4096" class="w-16 bg-gray-800 text-gray-100 rounded px-2 py-1 border border-gray-700 focus:outline-none" title={$LL.i2c.lenTitle()}>
      </div>
      <button onclick={doRead} disabled={!$i2cConnected || busy !== null} class="text-xs px-2 py-1 rounded bg-blue-700 hover:bg-blue-600 text-white disabled:opacity-40">{$LL.i2c.read()}</button>
    </div>
    ...
    <div class="flex flex-col gap-1.5 bg-gray-950 rounded p-2.5">
      ...
      <div class="flex gap-1.5 text-xs">
        ...
        <input type="number" bind:value={eepromOffset} min="0" max="65535" class="w-20 bg-gray-800 text-gray-100 rounded px-2 py-1 border border-gray-700 focus:outline-none" title={$LL.i2c.offsetTitle()}>
        <input type="number" bind:value={eepromLen} min="1" max="65535" class="w-16 bg-gray-800 text-gray-100 rounded px-2 py-1 border border-gray-700 focus:outline-none" title={$LL.i2c.eepromLenTitle()}>
      </div>
      <button onclick={doReadEeprom} disabled={!$i2cConnected || busy !== null} class="text-xs px-2 py-1 rounded bg-blue-700 hover:bg-blue-600 text-white disabled:opacity-40">{$LL.i2c.readEepromBtn()}</button>
    </div>
    ...
    <div class="flex flex-col gap-1.5 bg-gray-950 rounded p-2.5">
      ...
      <div class="flex gap-1.5 text-xs">
        ...
        <input bind:value={writeData} placeholder="01 02 03" class="flex-1 min-w-0 bg-gray-800 text-gray-100 rounded px-2 py-1 border border-gray-700 focus:outline-none font-mono" title={$LL.i2c.dataTitle()}>
      </div>
      <button onclick={doWrite} disabled={!$i2cConnected || busy !== null} class="text-xs px-2 py-1 rounded bg-amber-700 hover:bg-amber-600 text-white disabled:opacity-40">{$LL.i2c.writeBtn()}</button>
    </div>
  </div>
        ...
        {#each $i2cErrlogEntries as e (e.code + (e.timestamp ?? ''))}
          <div class="rounded bg-gray-800 border border-gray-700 p-2 text-xs">
            <div class="flex items-start justify-between gap-2">
              <span class="font-mono font-bold text-orange-400" title={$LL.i2c.xboxCodeTitle()}>{e.code}</span>
              {#if e.timestamp !== null}
  ...
  <div class="flex flex-col gap-1">
    <div class="relative">
      ...
      >
      {#if dbQuery}
        <button onclick={() => { dbQuery = ''; searchResults = []; if (debounceTimer) { clearTimeout(debounceTimer); debounceTimer = null; } }} title={$LL.i2c.searchResetTitle()} class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-300 text-xs">✕</button>
      {/if}
    </div>
