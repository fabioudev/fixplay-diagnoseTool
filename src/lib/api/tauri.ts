...
export const uartListPorts = (): Promise<UartPortInfo[]> =>
  invoke<UartPortInfo[]>('uart_list_ports');
...
export const flashWrite = (
  path: string,
  programmer: string,
  verify: boolean,
  allowInvalid = false
): Promise<void> => invoke<void>('flash_write', { path, programmer, verify, allowInvalid });
