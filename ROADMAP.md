...
- [x] **#109** In-app CH341A guide too broad for NOR work — slimmed the NOR-Flash panel guide down to the three things that matter: (1) mode-jumper explanation (gesteckt = Programmer-Modus, 2↔3 = USB-TTL), (2) new ZIF-socket pinout SVG with all 16 positions color-coded (SPI/3,3V/I²C/GND, "25 SPI"-Hälfte = Steckplätze 5–12 hervorgehoben) plus the chip↔ZIF wiring table, (3) SOIC-8 NOR pinout. Board top-view kept with only 3 callouts (Jumper/Sockel/Chip) + matching 3-entry legend. Dropped: SOIC-16 pinout, 3.3V-fix details, in-circuit prose, 11-entry legend, voltage jumper (blue board) — all of that lives in `docs/CH341A_GUIDE.md` (linked). 5V warning now only as the persistent strip above the guide (no duplicate block inside). Also fixed typesafe-i18n generation under TS 6 (ESM temp output imported from inside `node_modules` fails Node's package.json `type` resolution → tempPath moved outside via `.typesafe-i18n.json`).
- [x] **#110** UART guide was a bare CP2102 placeholder — rebuilt in the same realistic style as the NOR guide, around the CH341A the user actually has: (1) shared board top-view SVG now renders in both variants — USB-TTL mode (Jumper auf 2↔3, TX/RX/GND-Header-Pins teal hervorgehoben, 5V rot) for UART, programmer mode (Jumper auf 1↔2) for NOR; (2) neuer Verkabelungs-SVG mit CH341A-USB-TTL-Dongle (7-Pin-Header, Shunt auf 2–3) ↔ PS5-EMC-Pads (Pin 7 TX / 6 RX / 4 GND), gekreuzte Leitungen und eingezeichnetem Spannungsteiler (1k in Serie + 2k nach GND) auf der 5V-TX-Leitung, dazu eigenem Warnkasten; (3) neuer PS5-Mainboard-Location-SVG (schematisch, nicht maßstäblich): 24-Pin-Header neben Salina-PMIC + Duplikat am BT/WiFi-Modul, Callouts ①/②; (4) neuer 24-Pin-EMC-Header-Pinout-SVG (2×12 wie der ZIF-Sockel, alle Pins aus symbrkrs/ps5-uart: 5V/GND/A1-Strap/EMC-RX-TX/3V3/Titania-UART 11–14/I²C/RST#, unbestätigte Pins gedimmt, genutzte Pins 4/6/7 mit amber-Ringen, EMC-Bereich 5–8 in teal gestrichelter Box) mit Farblegende; (5) Prozedur um Jumper-2↔3-Schritt erweitert, Titania-Notiz mit Pins 11–14 + Baudraten (EFC 460800 / APU 230400 / EAP 691200). UartPanel-Section auf `overflow-y-auto` — der aufgeklappte Guide war höher als das Panel und wurde von main's overflow-hidden unerreichbar abgeschnitten.

## 🚀 Release-Pipeline

- [x] **#108** Release-Audit: 6 verifizierte Defekte in der Release-Pipeline (alle gegen die veröffentlichten v0.3.0-Artefakte nachgewiesen) — (1) Linux/macOS-Releases bundelten nur den 12-Byte-Platzhalter-Stub → Flash-Feature out-of-the-box kaputt: Runtime-Guard (`is_placeholder_binary` in fixplay-flashrom) erkennt den Stub, `resolve_flashrom_path` fällt auf PATH zurück, Selbst-Check meldet ihn; CI baut jetzt echtes flashrom v1.4.0 für Linux x86_64/ARM64 + macOS (gleiche CONFIG_*-Flags wie Windows), Arch-Paket liefert Distro-flashrom (+ `depends`), AUR-Source-PKGBUILD bündelt den Stub nicht mehr (+ `flashrom`-Abhängigkeit). (2) Windows-flashrom.exe dynamisch gelinkt → importiert libusb-1.0.dll/libwinpthread-1.dll, die der Installer nicht mitliefert (per objdump im veröffentlichten Installer bestätigt): `LDFLAGS="-static"` im MSYS2-Build. (3) SHA256SUMS.txt listete sich selbst (e3b0c442-Leerstring-Hash im veröffentlichten Release) → `! -name SHA256SUMS.txt` + `sha256sum -c` als Verifikation. (4) Tag↔tauri.conf.json-Version-Drift lief grün durch und erzeugte 404ende AUR-Pakete/leeres latest.json → Version-Gate im `prepare`-Job. (5) `release`-Job hing nicht von `build-extra` ab (AppImage-Checksums-Manifest fehlte dmg/ARM64-Artefakten). (6) Arch-Paket ohne xbox_error_codes.json und ohne JSON-Validitätsgate beim Error-DB-Download. (Unvalidated bis zum nächsten Tag-Release.)
